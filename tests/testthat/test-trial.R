...
test_that('event count per arm per endpoint can be dropped safely for shorter run time', {
  ...
  for(n_workers in 1:2){
    c1 <- tester(tidy = TRUE, n_workers = n_workers)
    c2 <- tester(tidy = FALSE, n_workers = n_workers)

    op11 <- c1$get_output()
    op12 <- c1$get_output(tidy = TRUE)

    op21 <- c2$get_output()
    op22 <- c2$get_output(tidy = TRUE)

    expect_true(identical(op12, op22)) # all columns except for event counts and milestone time
    for(col in names(op11)){
      expect_true(identical(op11[col], op21[col]))
    }
  }
})
