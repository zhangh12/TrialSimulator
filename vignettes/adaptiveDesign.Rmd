...
user-defined events. Family-wise error rate are controlled by closed
...
os <- endpoints(name = 'os', type = 'tte',
                   generator = rexp, rate = log(2) / 14)

five_weeks <- 5 / 52 * 12 ## convert it in months
surrogate <- endpoints(name = 'surrogate', type = 'non-tte',
...
pre-defined events later. Note that if `seed = NULL`, `TrialSimulator`
...
Next, we define action functions for trial events, i.e., dose selection,
interim and final analysis. Note that an action function should always
has arguments `trial` of class `Trial` and a character `event_name`. We
...
non-binding futility analysis in the following action function. epending
...
```{r}
dose_selection <- event(name = 'dose selection', action = action1, 
                        trigger_condition = 
                          eventNumber(endpoint = 'surrogate', n = 300)
                        )

interim <- event(name = 'interim', action = action2, 
                 trigger_condition = 
                   eventNumber(endpoint = 'pfs', n = 300)
                 )

final <- event(name = 'final', action = action3, 
               trigger_condition =
...
(through the `events` argument) automatically, thus, unlike in
`action3`, an explicit call of `Trial$get_locked_data(event_name)` is
unnecessary.
...
dose because it is dropped at dose selection. Thus, `event_at_reject` is
...
(`event_at_reject`). The `reject_time` (`r ct_os$reject_time[2]`) can be
...
output %>% 
  summarise(
    time_dose_selection = mean(`event_time_<dose selection>`), 
    time_interim = mean(`event_time_<interim>`), 
    time_final = mean(`event_time_<final>`), 
