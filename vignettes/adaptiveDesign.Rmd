...
In this vignette, we illustrate how to use `TrialSimulator` to develop
codes to simulate a trial with seamless design, where dose selection,
futility analysis, interim analysis are triggered by user-defined
events.
...
endpoint, VRR, is used for dose selection. We consider two active arms
(low or high dose) and a placebo arm with `1:1:1` randomization. In the
placebo arm, the median of PFS and OS are 5.45 and 13.71 months,
respectively. The binary endpoint `VRR` is of a rate 0.01. By default,
the permuted block randomization is executed.
...
Similarly, we can define OS and VRR for placebo. Here built-in functions
`rexp` and `rbinom` are used. User-defined functions are also supported
as long as it has the first argument `n`, number of samples to be drawn,
and returns a data frame of `n` rows. For any time-to-event endpoint
(e.g., `os`) in the data frame, `TrialSimulator` expects a column ending
with `_event` (e.g., `os_event`) being present as the event indicator.
In general, user can define their own generator that returns more than
one endpoints. For example, correlated endpoints would be generated
together, using packages i.e., `simstudy`, `gestate`, `simdata`, etc.
Note that VRR is not a time-to-event endpoint, so a readout time
(`vrr_readout`) is also needed when creating the endpoint. In this
example, the readout time is 6 weeks. Once all endpoints are defined, an
arm can be defined using class `Arm` and its method `new`.
...
six_weeks <- 6 / 52 * 12
vrr <- Endpoint$new(
  name = 'vrr', type = 'binary', readout = c(vrr = six_weeks), 
...
With three arms, we can define a trial of class `Trial`, where up to 950
patients are enrolled. Recruitment curve are specified through
...
dropout rate 20% per 12 months. By default, no dropout if users do not
specify one.

```{r lagieg, class.source="fold-show"}
accrual_rate <- data.frame(end_time = c(15, Inf), piecewise_rate = c(20, 25))
trial <- Trial$new(
  name = 'Trial-3415', n_patients = 950,
  seed = 31415926, duration = 64,
  enroller = StaggeredRecruiter, accrual_rate = accrual_rate, 
  dropout = rexp, rate = -log(1 - 0.2)/12
)
...
done at month 15. A built-in function `TriggerByCalendarTime` is used as
...
dose_selection_action <- function(trial, event_name){
  ...
  trial$save(value = affected_arm, name = 'removed_arm')
  trial$save(value = z_pfs, name = 'z_futility')
  
  ## Not necessary because event time is saved automatically
...
## dose selection is carried out at month 15
dose_selection_event <- Event$new(name = 'dose selection', type = 'dose selection',
                                  ...
                                  calendar_time = 15 + six_weeks)
```

Likewise, we can define events for interim and final analyses of PFS and
OS. The interim analysis for PFS will be carried out when 140 events are
...
420 events are observed for PFS, or 262 events for OS, whichever comes
first.
...
pfs_final_action <- function(trial, event_name){
  ...
  trial$save(trial$get_event_time(event_name), paste0('event_time_', event_name))
  trial$save(
...
## when pfs event reaches 420 or os event reaches 262.
...
pfs_final_event <- Event$new(name = 'pfs final', type = 'pfs final/os interim',
                               ...
                               endpoints = c('pfs', 'os'),
                               target_n_events = c(420, 262), meet = 'any')
```
...
In the action functions, user can save results to the `Trial` object using 
its public member function `save` or
any place they chose so that they can summarize the trial later. 
The package can also summarize commonly used metrics by default. 
For common design, the package can carry out standard analysis, e.g., 
group sequential design with combination test or graphical test as well, which would be implemented and tested later.
...
management to allow user focus on implementing the statistical
