...
Similarly, we can define OS and VRR for placebo. Here builtin functions
...
With three arms, we can define a trial of class `Trial`, where up to 750
patients are enrolled. Recruitment curve are specified through
`enroller` with a builtin function `StaggeredRecruiter` of piecewise
constant rate: 15 patients per month until 14 months, and then 20
patients per month until the end of the trial. Builtin random number
...
updated during a trial using method `update_sample_ratio`.
...
done at month 14. A builtin function `TriggerByCalendarTime` is used as
...
OS. The interim analysis for PFS will be carried out when 110 events are
observed.
...
331 events are observed for PFS, or 207 events for OS, whichever comes
first.
