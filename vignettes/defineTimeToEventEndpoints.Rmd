...
- `endpoints`: Creates one or more endpoints
- `test_generator`: Generates an example dataset from an Endpoint object

Typically, endpoints follow the same distribution family across arms but with different parameters. Distribution parameters are specified through the `...` arguments in `endpoints`.
...
```{r lkioie}
pfs_pbo <- endpoints(name = 'PFS', type = 'tte', 
...
```{r aldj}
library(TrialSimulator)
pfs_trt <- endpoints(name = 'PFS', type = 'tte', 
...
```{r laiojb}
os_pbo <- endpoints(name = 'OS', type = 'tte', 
                       generator = rexp, rate = log(2)/7.2)
os_trt <- endpoints(name = 'OS', type = 'tte', 
...
Note that by defining `PFS` and `OS` separately (i.e., calling `endpoints` for each endpoint), you are implicitly assuming that these endpoints are independent in the trial.
...
eps_pbo <- endpoints(name = c('PFS', 'OS'), type = c('tte', 'tte'), 
                              generator = custom_generator, 
                              pfs_rate = log(2)/5.6, os_rate = log(2)/7.2, 
                              corr = corr)

eps_trt <- endpoints(name = c('OS', 'PFS'), type = c('tte', 'tte'), 
                              generator = custom_generator, 
