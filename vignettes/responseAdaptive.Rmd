...
```{r ref.label='stage_action', eval=FALSE}
```
...
```{r ref.label='final_action', eval=FALSE}
```
...
In the output, the columns `n_event_<milestone>_<arms>` contain detailed information on observed events or sample sizes per arm at each milestone. It is evident that we have pipeline patients at both interim. 
...
For completeness, the full code of the helper functions `compute_sample_ratio()` and `multiple_contrast_test()` is included below, which determine the new sample ratio and performs the multiple contrast test. Note that the implementation to these two functions are completely project-specific. 

```{r ref.label='helpers', eval=FALSE}
```
