...
Arms <- R6::R6Class(
  ...
  public = list(
    ...
    #' It must not contain \code{'@'} or \code{';'}, which are reserved for
    #' encoding the treatment switching history (\code{regimen_trajectory})
    #' of trials with a regimen.
    ...
    initialize = function(name, ...){
      ...
      if(grepl('[@;]', name)){
        stop('Arm name <', name, '> must not contain \'@\' or \';\'. ',
             'These characters are reserved for encoding the treatment ',
             'switching history (regimen_trajectory) in trial data. ')
      }
