...
#' function is called (i.e., the current milestone), not from the start of
#' the trial. A milestone is usually event driven, so its calendar time is
#' unknown until the trial is simulated, and a schedule on the calendar time
#' scale could not be pre-specified. Measuring \code{end_time} from the
#' milestone also lets users state the new plan simply as "from now on":
#' e.g., \code{data.frame(end_time = c(3, Inf), piecewise_rate = c(20, 35))}
#' means 20 patients per month for the 3 months following the milestone and
#' 35 per month thereafter, whenever the milestone occurs. Following the
#' convention of \code{StaggeredRecruiter}, the first re-planned patient is
#' enrolled \code{1/piecewise_rate} after the milestone; a leading window
#' with \code{piecewise_rate = 0} defers enrollment further. As with other
...
#' \code{Trials$update_accrual_rate()}, which is used in vignettes. Users
#' who are not familiar with the concept of classes may consider using this
#' wrapper directly.
#'
...
update_accrual_rate <- function(trial, accrual_rate){
  ...
  trial$update_accrual_rate(accrual_rate = accrual_rate)
