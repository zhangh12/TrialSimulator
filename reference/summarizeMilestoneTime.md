...
- output:
  ...
  a data frame. It assumes that triggering time of milestones are store
  in columns `milestone_time_<...>`. It can be a data frame returned by
  `controller$get_output()`, or row-binded from multiple data frames
  returned by `controller$get_output()` (e.g., users may run simulation
  under the `targets` framework).

## Value

A data frame of class `milestone_time_summary`. It comes with a `plot`
...
#> 1 final analysis 15.88651 15.85284 0.197101 10
