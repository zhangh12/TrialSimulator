...
- `trial$add_arms()` registration is now transactional: all incoming arms are validated (including endpoint-set consistency) before any of them is installed, so a failing batch no longer leaves the trial partially modified.
